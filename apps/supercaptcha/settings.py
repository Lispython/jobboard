...
HTML_TEMPLATE_WITH_REFRESH = getattr(settings, 'CAPTCHA_HTML_TEMPLATE_WITH_REFRESH',
                                     """
                                     <img src="%(src)s?%(rnd)s" alt="%(alt)s" width="%(width)s" height="%(height)s" />
                                     <a onclick="var img=this.previousSibling;if(img.nodeType==document.TEXT_NODE){img=img.previousSibling};img.src=img.src.substring(0,img.src.indexOf('?')+1)+Math.random();return false;" href="#refresh" class="supercaptcha-refresh">%(refresh_text)s</a>
                                     <input%(input_attrs)s maxlength="%(length)s" />
                                     """)
